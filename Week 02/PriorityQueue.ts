...
export default class PriorityQueue<T = number> {
  ...
  constructor(compareFn: (a: T, b: T) => number, data: T[] = []) {
  ...
  inRange = (idx: number) => idx >= 1 && idx <= this.lastIdx;
  /** 返回父节点索引，如果是根节点，返回null */
  parentIdx = (idx: number): number | null =>
    idx === 1 ? null : Math.floor(idx / 2);

  /**
   * 用于判断某个节点的左右孩子哪个值比较「大」
   * 叶子节点返回null
   */
  childIdx = (idx: number): number | null => {
    const left = idx * 2;
    const right = idx * 2 + 1;
