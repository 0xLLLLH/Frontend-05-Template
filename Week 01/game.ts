...
export default class Game {
  ...
  PlayerTurnOrder: [Player, Player] = [HumanPlayer, AIPlayer];
  ...
  constructor(
    ...
    readonly BoardSize: number = 3,
    readonly WinCount: number = 3
  ) {
  ...
  renderBoard = () => {
    ...
    this.board.appendChild(
      document.createTextNode(
        `第${this.currentTurn + 1}回合，轮到${
          this.PlayerTurnOrder[this.currentTurn % 2].name
        }操作`
      )
    );
    ...
    for (let i = 0; i < this.BoardSize; i++) {
      for (let j = 0; j < this.BoardSize; j++) {
        ...
        square.appendChild(
          document.createTextNode(BoardTokenMap[this.pattern[i][j]] || "")
        );
        square.addEventListener("click", () => {
          if (
            this.PlayerTurnOrder[this.currentTurn % 2].value ===
              HumanPlayer.value &&
            !this.pattern[i][j]
          ) {
  ...
  countByDirection = (
    ...
    const byVector = (vector: Vector) => {
      ...
      while (this.posInRange(nextPos)) {
        if (
          pattern[startPos.row][startPos.col] ===
   ...
   * @param point 最后下的位置
   */
  checkWin(pattern: Board, point: Point): boolean {
   ...
   * 目前采用比较简单的评分方式
   */
  evalPattern = (pattern: Board, player: Player) => {
    const Base = 10;
    let score = 0;
    ...
    for (let i = 0; i < this.BoardSize; i++) {
      for (let j = 0; j < this.BoardSize; j++) {
        ...
        for (let dir of Game.directions) {
          ...
          const [count, spaceCount] = this.countByDirection(
            pattern,
            { row: i, col: j },
            dir
          );
  ...
  findBestChoice = (
    ...
    // 让己方局面最优的位置及分数
    let maxChoice: [number, Point] | null = null;
    let win = false;
    // 为了缩小规模只考虑已有棋子附近的位置
    const notSoFar = (p: Point) => {
      for (let dir of Game.directions) {
